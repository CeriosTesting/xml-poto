...
const expensiveProducts = catalog.query.find("Product").whereValueGreaterThan(100);
```
...
const expensiveBooks = library.booksQuery?.filter((book) => parseFloat(book.attr("price") || "0") > 50);
```
...
// Walk up with filter
const filteredAncestors = query
	.find("Product")
	.first()
	?.walkUp((el) => el.attributes.type === "container");
...
const products = new XmlQuery([catalog.query]).breadthFirst().where((el) => el.name === "Product");

// Get all elements following the computers category
const computersCategory = catalog.query.xpath('//Category[@id="computers"]').first();
const following = computersCategory ? new XmlQuery([computersCategory]).followingNodes().toArray() : [];
...
const leafNodes = new XmlQuery([catalog.query]).depthFirst().where((el) => el.isLeaf);
```
...
query.find("Product").whereAttributePredicate("price", (price) => parseFloat(price) > 100);
...
query.find("Description").whereTextPredicate((text) => text.length > 100);
...
query
	.find("Product")
	.where((element, index) => element.attributes.stock && parseInt(element.attributes.stock) > 0 && index < 10);
...
// Check conditions
const allExpensive = query.find("Product").all((p) => parseFloat(p.attributes.price || "0") > 100);

const anyExpensive = query.find("Product").any((p) => parseFloat(p.attributes.price || "0") > 100);
...
const totalValue = query.find("Price").reduce((sum, p) => sum + (p.numericValue || 0), 0);
...
const uniqueCategories = query.find("Product").distinctBy((p) => p.attributes.category);
...
const grouped = query
	.find("Product")
	.groupBy((p) => (parseFloat(p.attributes.price || "0") > 100 ? "expensive" : "affordable"));
...
// Find in specific namespace
const userId = doc.query.inNamespace("http://example.com/app", "UserId");
...
const tsArticles = feed.query
	.find("item")
	.filter((item) => item.children.find((c) => c.name === "category")?.text === "TypeScript")
	.map((item) => ({
...
const byCategory = feed.query
	.find("item")
	.groupBy((item) => item.children.find((c) => c.name === "category")?.text || "Uncategorized");
```
...
const settings = config.query.find("Setting").toMap(
	(s) => s.attributes.key,
...
const hasAuth = envelope.query.localName("Authentication").exists();
```
